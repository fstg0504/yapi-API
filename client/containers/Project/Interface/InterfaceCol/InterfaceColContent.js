            ...
            {this.state.hasPlugin ? (
              ...
              >
                {this.props.curProjectRole !== 'guest' && (
                  <Tooltip title="在 APIDoc 服务端跑自动化测试，测试环境不能为私有网络，请确保 APIDoc 服务器可以访问到自动化测试环境domain">
                    <Button
                      style={{
                        marginRight: '8px'
                      }}
          ...
          >
            ...
            <Row type="flex" justify="space-around" className="row" align="middle">
              <Col span={21} className="autoTestUrl">
                <a href={localUrl + autoTestsUrl} target="_blank" rel="noopener noreferrer">
                  {autoTestsUrl}
                </a>
              </Col>
              <Col span={3}>
            ...
            <div className="autoTestMsg">
              注：访问该URL，可以测试所有用例，请确保APIDoc服务器可以访问到环境配置的 domain
            </div>
          </Modal>
        )}
      </div>
