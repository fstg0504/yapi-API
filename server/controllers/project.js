...
class projectController extends baseController {
  ...
  async add(ctx) {
    ...
    yapi.commons.saveLog({
      content: `${username} 添加了项目 <a href="/project/${
        result._id
      }">${params.name}</a>`,
      type: 'project',
      uid,
    ...
    for (let i = 0, len = params.member_uids.length; i < len; i++) {
      ...
      if (!userdata) {
        no_members.push(id);
          ...
          let check = await this.Model.checkMemberRepeat(params.id, userdata.uid);
          if (check>0){
  ...
  async delMember(ctx) {
    ...
    var check = await this.Model.checkMemberRepeat(params.id, userdata.uid);
    if (check === 0) {
    ...
    let result = await this.Model.delMember(params.id, userdata.uid);
    let username = this.getUsername();
    yapi
      .getInst(userModel)
      ...
      .then(member => {
        yapi.commons.saveLog({
          content: `${username} 删除了项目中的成员 ${member.username}`,
          type: 'project',
          uid: this.getUid(),
          username: username,
          typeid: params.id
  ...
  async up(ctx) {
    try {
      ...
      let username = this.getUsername();
      yapi.commons.saveLog({
        content: `${username} 更新了项目 <a href="/project/${id}/interface/api">${
          projectData.name
        }</a>`,
        type: 'project',
        uid: this.getUid(),
